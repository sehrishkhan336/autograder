...
def _extract_structural_fingerprint(sql: str) -> Dict[str, bool]:
    ...
    fingerprint = {
        "has_where": bool(re.search(r"\bwhere\b", clean)),
        ...
        "has_implicit_join": bool(re.search(r"\bfrom\s+\w+\s*,\s*\w+", clean)),
        "has_having": bool(re.search(r"\bhaving\b", clean)),
        "has_order_by": bool(re.search(r"\border\s+by\b", clean)),
        ...
        "has_insert_into": bool(re.search(r"\binsert\s+into\b", clean)),
        "has_update": bool(re.search(r"\bupdate\b", clean)),
        "has_delete": bool(re.search(r"\bdelete\b", clean)),
        "has_create_table": bool(re.search(r"\bcreate\s+table\b", clean)),
        "has_alter_table": bool(re.search(r"\balter\s+table\b", clean)),
        "has_declare": bool(re.search(r"\bdeclare\b", clean)),
        "has_set": bool(re.search(r"\bset\b", clean)),
        "has_create_func": bool(re.search(r"\bcreate\s+function\b", clean)),
        "has_create_proc": bool(re.search(r"\bcreate\s+proc(?:edure)?\b", clean)),
